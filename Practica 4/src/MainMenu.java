...
public class MainMenu {
	...
	public static void menuNuevoUsuario() { //Meno de creación de Usuario
		...
		System.out.print("Meta su contrasena: ");
	...
	public static void menuInicioDeSesion() { //Metodo inicio de sesión
		...
		case 1:
			...
			String apeUsuario = scn.nextLine();
			scn.nextLine();
			System.out.println("Meta su contrasena: ");
			String conUsuario = scn.nextLine();
			scn.nextLine();
			
			apeUsuario = apeUsuario.replaceAll(" ", "");
			
			boolean existeUsuario = u.iniciarSesion(nomUsuario, apeUsuario, conUsuario);
			if(existeUsuario && u.getTipoDeLicencia(nomUsuario, apeUsuario).contentEquals("Hotelero")) { //Comprobamos si el usuario existe o es correcto y comprobamos su licencia
				menuInicioHotelero(nomUsuario, apeUsuario); 																							//Si es hotelero le llevamos al menú para hoteleros
			}else if(existeUsuario && u.getTipoDeLicencia(nomUsuario, apeUsuario).contentEquals("Cliente")){
				menuInicioCliente(nomUsuario, apeUsuario);																								//Si es cliente le mandamos al menú para clientes
			}else if(existeUsuario && u.getTipoDeLicencia(nomUsuario, apeUsuario).contentEquals("Admin")) {
				menuAdministrador(nomUsuario, apeUsuario);
			}else{
	...
	public static void menuInicioDeSesion(Usuario p) {  //metodo sobrecargado menuInicioDeSesion, para mandar al nuevo usuario directamente al menu principal
		...
		if(auxTernario) {
			...
			if(p.getTipoDeLicencia().contentEquals("Admin")) {
				menuAdministrador(p.getNombreUsuario(), p.getApellidoUsuario());
			}else {
				menuInicioCliente(p.getNombreUsuario(),p.getApellidoUsuario());
			}
		}
	}
	...
	public static void menuAdministrador(String nomUsuario, String apeUsuario){
		Scanner scn = new Scanner(System.in);
		int decision = 0;
		
		Administrador ad = new Administrador();
		
		boolean existeContrasenaAdmin = ad.conAdminExiste(nomUsuario, apeUsuario);
		
		if(!existeContrasenaAdmin) {
			try {
				ad.addContrasenaBorrado(nomUsuario, apeUsuario);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		System.out.println("Que quieres hacer?: ");
		System.out.println("1. Borrar todo el sistema");
		...
		case 1:
			boolean aux = false;
			System.out.println("Meta su contrasena de administrador(No la de usuario): ");
			scn.nextLine();
			String con = scn.next();
			try {
				aux = ad.comprobarContrasenaBorrado(nomUsuario, apeUsuario, con);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if(aux) {
				boolean aux2 = ad.borrarEHotel();
				if(aux2) {
				...
				System.out.println("Ha ocurrido algo, no se ha podido borrar");
				menuAdministrador(nomUsuario, apeUsuario);
			}
			break;
		 default:
	...
	public static void menuNuevaHabitacion(String nomUsuario, String apeUsuario){
		...
		System.out.println("Meta el nombre del hotel al que quiere anadir una habitacion: ");
	...
	public static void menuNuevoHotel(String nomUsuario, String apeUsuario) {	//Menú crear nuevo hotel
		Scanner scn = new Scanner(System.in);
		System.out.println("¿Estas seguro de que quieres anadir un nuevo hotel?: ");
		System.out.println("1. No");
